...
void StaticIP(wiredNetworkConfig_t wiredNetworkconfig)
{
    dhcp_stop(netif_default);
    netifapi_netif_set_addr(netif_default, &wiredNetworkconfig.ipaddr, &wiredNetworkconfig.netmask, &wiredNetworkconfig.gw);
    dns_setserver(0, &wiredNetworkconfig.dns1);
    dns_setserver(1, &wiredNetworkconfig.dns2);
}
